/*
	...
	solution is plotted against the timestep size to test the convergence of the method.
...
/*		EXECUTE CONVERGENCE TEST
	...
	createConvergenceRunInfo(myGridType,myInterpScheme,"TerzaghiT");
	for(int i=0; i<meshSizeNo; i++)
	{
		h=1./meshSize[i];
		exportConvergenceRunInfo(h,"TerzaghiT");

		for(int j=0; j<timeStepsNo; j++)
		{
			ierr=convergence(myGridType,myInterpScheme,timeSteps[j],meshSize[i],Lt,g,sigmab,
