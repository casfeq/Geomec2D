...
plotName="plot/doublePorosity_h_stability.png"
...
for j in range(0,len(gridType)):
	...
	# Define figure's name
	plotName="plot/doublePorosity_stripfoot_h_stability_"+gridType[j]+"-grid.png"
	...
	# Plot micro-pressure 1
	fileName=str(parentDirectory)+"/export/stripfoot_"+solvedPairs[0]+"_PNumeric_h="+format(h[0], \
		".6f")+"_timeStep=1_"+gridType[j]+"-grid.txt"
	pNumeric=np.loadtxt(fname=fileName)
	pNumeric[:]=[x/1000 for x in pNumeric]
	fileName=str(parentDirectory)+"/export/stripfoot_"+solvedPairs[0]+"_xCoord_h="+format(h[0], \
		".6f")+"_timeStep=1_"+gridType[j]+"-grid.txt"
	xPosition=np.loadtxt(fname=fileName)
	fileName=str(parentDirectory)+"/export/stripfoot_"+solvedPairs[0]+"_yCoord_h="+format(h[0], \
		".6f")+"_timeStep=1_"+gridType[j]+"-grid.txt"
	yPosition=np.loadtxt(fname=fileName)
	XPosition,YPosition=np.meshgrid(xPosition,yPosition)
	plt.contourf(XPosition,YPosition,pNumeric,1000,cmap=cm.inferno)
	...
	# Plot micro-pressure 2
	fileName=str(parentDirectory)+"/export/stripfoot_"+solvedPairs[0]+"_PNumeric_h="+format(h[1], \
		".6f")+"_timeStep=1_"+gridType[j]+"-grid.txt"
	pNumeric=np.loadtxt(fname=fileName)
	pNumeric[:]=[x/1000 for x in pNumeric]
	fileName=str(parentDirectory)+"/export/stripfoot_"+solvedPairs[0]+"_xCoord_h="+format(h[1], \
		".6f")+"_timeStep=1_"+gridType[j]+"-grid.txt"
	xPosition=np.loadtxt(fname=fileName)
	fileName=str(parentDirectory)+"/export/stripfoot_"+solvedPairs[0]+"_yCoord_h="+format(h[1], \
		".6f")+"_timeStep=1_"+gridType[j]+"-grid.txt"
	yPosition=np.loadtxt(fname=fileName)
	XPosition,YPosition=np.meshgrid(xPosition,yPosition)
	plt.contourf(XPosition,YPosition,pNumeric,1000,cmap=cm.inferno)
	...
	# Plot macro-pressure 1
	fileName=str(parentDirectory)+"/export/stripfoot_"+solvedPairs[0]+"_MacroPNumeric_h="+ \
		format(h[0],".6f")+"_timeStep=1_"+gridType[j]+"-grid.txt"
	pNumeric=np.loadtxt(fname=fileName)
	pNumeric[:]=[x/1000 for x in pNumeric]
	fileName=str(parentDirectory)+"/export/stripfoot_"+solvedPairs[0]+"_xCoord_h="+format(h[0], \
