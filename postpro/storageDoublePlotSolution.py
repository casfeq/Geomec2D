...
# Get dt
fileName=str(parentDirectory)+"/export/solveStorageDouble_"+solvedPairs[0]+"RunInfo.txt"
...
plotName="plot/storageDouble.png"
...
for j in range(0,len(gridType)):
	for i in range(0,len(timesteps)):
		fileName=str(parentDirectory)+"/export/storageDouble_"+solvedPairs[0]+"_PPoreExact_dt="+ \
		...
		fileName=str(parentDirectory)+"/export/storageDouble_"+solvedPairs[0]+"_YExact_dt="+ \
			format(dt,".6f")+"_timeStep="+str(timesteps[i])+"_"+gridType[j]+"-grid.txt"
		...
		if j==0:
			exact,=plt.plot(pExact,yExact,'-',color='grey',fillstyle='none',linewidth=1.25)
		fileName=str(parentDirectory)+"/export/drainedDouble_"+solvedPairs[0]+"_PPoreNumeric_dt=" \
			+format(dt,".6f")+"_timeStep="+str(timesteps[i])+"_"+gridType[j]+"-grid.txt"
		pNumeric=np.loadtxt(fname=fileName)
		pNumeric[:]=[x/1000 for x in pNumeric]
		fileName=str(parentDirectory)+"/export/drainedDouble_"+solvedPairs[0]+"_YPNumeric_dt="+ \
			format(dt,".6f")+"_timeStep="+str(timesteps[i])+"_"+gridType[j]+"-grid.txt"
...
for j in range(0,len(gridType)):
	for i in range(0,len(timesteps)):
		fileName=str(parentDirectory)+"/export/storageDouble_"+solvedPairs[0]+"_PFracExact_dt="+ \
			format(dt,".6f")+"_timeStep="+str(timesteps[i])+"_"+gridType[j]+"-grid.txt"
		pExact=np.loadtxt(fname=fileName)
		pExact[:]=[x/1000 for x in pExact]
		fileName=str(parentDirectory)+"/export/storageDouble_"+solvedPairs[0]+"_YExact_dt="+ \
		...
		if j==0:
			exact,=plt.plot(pExact,yExact,'-',color='grey',fillstyle='none',linewidth=1.25)
		fileName=str(parentDirectory)+"/export/drainedDouble_"+solvedPairs[0]+"_PFracNumeric_dt=" \
			+format(dt,".6f")+"_timeStep="+str(timesteps[i])+"_"+gridType[j]+"-grid.txt"
		pNumeric=np.loadtxt(fname=fileName)
		pNumeric[:]=[x/1000 for x in pNumeric]
		fileName=str(parentDirectory)+"/export/drainedDouble_"+solvedPairs[0]+"_YPNumeric_dt="+ \
...
print("Plotted Drained Column (Double Porosity / Storage)")