...
for j in range(0,len(gridType)):
	for i in range(0,len(timesteps)):
		...
		if i==0 and j==0:
			exact,=plt.plot(xExact,pExact,'-',color='grey',fillstyle='none',linewidth=1.0, \
				label="Analytical")
		if j==0:
			exact,=plt.plot(xExact,pExact,'-',color='grey',fillstyle='none',linewidth=1.0)
		...
		if i==0:
			numeric,=plt.plot(xNumeric,pNumeric,markers[j],fillstyle='none',ms=5,mew=0.75, \
				label=labels[j])
		else:
			numeric,=plt.plot(xNumeric,pNumeric,markers[j],fillstyle='none',ms=5,mew=0.75)
...
for j in range(0,len(gridType)):
	for i in range(0,len(timesteps)):
		...
		xExact=np.loadtxt(fname=fileName)
		if j==0:
			exact,=plt.plot(xExact,epsExact,'-',color='grey',fillstyle='none',linewidth=1.0)
		...
		fileName=str(parentDirectory)+"/export/mandel_"+solvedPairs[0]+"_XENumeric_dt="+ \
			format(dt,".6f")+"_timeStep="+str(timesteps[i])+"_"+gridType[j]+"-grid.txt"
		xNumeric=np.loadtxt(fname=fileName)
		numeric,=plt.plot(xNumeric,eNumeric,markers[j],fillstyle='none',ms=5,mew=0.75)
...
for j in range(0,len(gridType)):
	for i in range(0,len(timesteps)):
		...
		xExact=np.loadtxt(fname=fileName)
		if j==0:
			exact,=plt.plot(xExact,uExact,'-',color='grey',fillstyle='none',linewidth=1.0)
		...
		fileName=str(parentDirectory)+"/export/mandel_"+solvedPairs[0]+"_XUNumeric_dt="+ \
			format(dt,".6f")+"_timeStep="+str(timesteps[i])+"_"+gridType[j]+"-grid.txt"
		xNumeric=np.loadtxt(fname=fileName)
		numeric,=plt.plot(xNumeric,uNumeric,markers[j],fillstyle='none',ms=5,mew=0.75)
