...
# Define figure's name
# plotName="plot/terzaghiDimless_e_"+gridType+"-grid.png"
...
# 	# Loop for plotting each pair
# 	for j in range(0,len(solvedPairs)):
# 		# Get pair runInfo
# 		fileName=str(parentDirectory)+"/export/solveTerzaghi_"+solvedPairs[j]+"RunInfo.txt"
# 		dtl=np.loadtxt(fname=fileName)
# 		fileName=str(parentDirectory)+"/input/"+solvedPairs[j]+".txt"
# 		# Plot numerical
# 		fileName=str(parentDirectory)+"/export/terzaghi_"+solvedPairs[j]+"_ENumeric_dt="+ \
# 			format(dtl[i],".6f")+"_timeStep=1_"+gridType+"-grid.txt"
# 		eNumeric=np.loadtxt(fname=fileName)
# 		eRef=eNumeric[len(eNumeric)-1]
# 		eNumeric[:]=[x/eRef for x in eNumeric]
# 		fileName=str(parentDirectory)+"/export/terzaghi_"+solvedPairs[j]+"_YENumeric_dt="+ \
# 			format(dtl[i],".6f")+"_timeStep=1_"+gridType+"-grid.txt"
# 		yNumeric=np.loadtxt(fname=fileName)
# 		yNumeric[:]=[x/6 for x in yNumeric]
# 		myFile=open(str(parentDirectory)+"/input/"+solvedPairs[j]+".txt","r")
...
# 		myFile.close()
# 		if i==0:
# 			numeric,=plt.plot(eNumeric,yNumeric,':.',color=colors[j],ms=10,mec='k',mew=0.5, \
# 				label=str(myLabel))
# 		else:
# 			numeric,=plt.plot(eNumeric,yNumeric,':.',color=colors[j],ms=10,mec='k',mew=0.5)
...
# 	# Set axes' labels
# 	plt.xlabel('Normalized Volumetric Strain')
