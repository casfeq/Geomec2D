...
# Get dt
fileName=str(parentDirectory)+"/export/solveLeakingDouble_"+solvedPairs[0]+"RunInfo.txt"
...
plotName="plot/leakingDouble.png"
...
for j in range(0,len(gridType)):
	for i in range(0,len(timesteps)):
		fileName=str(parentDirectory)+"/export/leakingDouble_"+solvedPairs[0]+"_PPoreExact_dt="+ \
		...
		fileName=str(parentDirectory)+"/export/leakingDouble_"+solvedPairs[0]+"_YExact_dt="+ \
			format(dt,".6f")+"_timeStep="+str(timesteps[i])+"_"+gridType[j]+"-grid.txt"
...
for j in range(0,len(gridType)):
	for i in range(0,len(timesteps)):
		fileName=str(parentDirectory)+"/export/leakingDouble_"+solvedPairs[0]+"_PFracExact_dt="+ \
			format(dt,".6f")+"_timeStep="+str(timesteps[i])+"_"+gridType[j]+"-grid.txt"
		pExact=np.loadtxt(fname=fileName)
		pExact[:]=[x/1000 for x in pExact]
		fileName=str(parentDirectory)+"/export/leakingDouble_"+solvedPairs[0]+"_YExact_dt="+ \
...
print("Plotted Drained Column (Double Porosity / Leaking)")